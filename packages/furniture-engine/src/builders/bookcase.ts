import {
  BookcaseConfiguration,
  FurnitureComponent,
  FurnitureLayout,
} from '@4basearch/furniture-types';

function rowInteriorWidth(
  row: BookcaseConfiguration['rows'][number],
  thickness: number,
): number {
  const columnsWidth = row.columns.reduce(
...
export function buildBookcaseLayout(
  ...
  const components: FurnitureComponent[] = [
    {
      type: 'panel',
      position: {
        x: -caseOuterWidth / 2 + thickness / 2,
        y: caseOuterHeight / 2,
        z: -depth / 2,
      },
    ...
    },
    {
      ...
      dimensions: {
        width: caseOuterWidth,
        height: caseOuterHeight,
        depth: backThickness,
      },
      material,
    },
